...
instagram.get('users/self/media/recent',{count:'30'}).then((data) => {
    let stringify = JSON.stringify(data.data);
   let parsed = JSON.parse(stringify);
// / parsed.replace(/[^\x00-\x7F]/g, "");

for (let i = 0; i < parsed.length; i++) {
console.log(parsed);
    let time = moment(Date.now()).format('YYYY-MM-DD HH:mm:ss');
    let data = [['0',parsed[i].caption.id,parsed[i].link,parsed[i].caption.text,parsed[i].created_time,parsed[i].likes.count,'Instagram',time]];
    ...
    connection.query(sql, [data], function (err, result) {
        if (err) throw err;
        console.log("The following number of rows have been added" + result);
    });
}
